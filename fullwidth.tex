...
\version{\fwdversion}
\date{\today}
\introduction{The packages provide the environment \fwdname which allows to set the left and right margins very simple. It also allows page breas. If you are using the \texttt{twoside}-mode you can set the margins  of the inner and outer. \par
...
The package itself loads the packages \mdpack{kvoptions}, \mdpack{etoolbox}.
...
I added a detection of \mdpack{float} or \mdpack{minipage} environments. If you use \fwdname in such an environment \fwdname will use the option \mdoption{nobreak} automaticly. 
...
All lengths will be evaluated by the \eTeX-commands \mdcommand{dimexpr}.
...
\mdDescribeMacro[true]{twosidemode} The package detected the \mdcommand{twoside} mode and uses \mdoption{innermargin} and \mdoption{outermagin} default. If you don't this you can use this option.
...
\mdDescribeMacro[\mbox{} \mdcommand{bigskipamount}]{footnotedistance} The length is the distance between the end of the enthronement \fwdname and the displaying of the \mdcommand{footnoterule}.
